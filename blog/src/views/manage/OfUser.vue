  ...
  <div class="total">
    ...
    >
      <div class="demo-drawer__content">
        <el-form :model="form" ref="form">
          <el-form-item label="账号" :label-width="formLabelWidth" prop="title">
            <el-input
              v-model="form.userid"
              autocomplete="off"
              disabled
    ...
    <div style="padding: 20px">
      ...
      >
        ...
        <el-table-column label="权限" prop="perms" sortable></el-table-column>
...
export default {
  ...
  methods: {
    load() {
      request
        .get("http://" + window.server.ip + ":9090/user", {
          params: {
    ...
    listDelete() {
      ...
      request
        .delete(
          "http://" + window.server.ip + ":9090/user/deleteIds?selection=" + res
        )
        .then((res) => {
          if (res.code === "0") {
            this.$message({
    ...
    handleDelete(userid) {
      request
        .delete("http://" + window.server.ip + ":9090/user/" + userid)
        .then((res) => {
          if (res.code === "0") {
            this.$message({
              type: "success",
